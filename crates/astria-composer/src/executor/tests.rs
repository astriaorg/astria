...
// Mounts a `CometBFT` status response with only a specified mock sequencer chain id
...
async fn full_bundle() {
    ...
    mount_partial_cometbft_status_response(&sequencer, &cfg.sequencer_chain_id).await;
...
async fn should_exit_if_mismatch_sequencer_chain_id() {
    ...
    mount_partial_cometbft_status_response(&sequencer, "bad-chain-id").await;
    ...
    assert!(executor.check_chain_ids().await.is_err());
}
