...
mod sized_bundle_tests {
    ...
    fn push_works() {
        ...
        let sized_seq_action = bundle.push_check(seq_action).unwrap();

        assert_eq!(seq_action_size, 100);
        bundle.push(sized_seq_action);
    }
    ...
    fn push_not_enough_space() {
        ...
        let sized_initial_seq_action = bundle.push_check(initial_seq_action).unwrap();
        bundle.push(sized_initial_seq_action);
        ...
        assert!(estimate_size_of_sequence_action(&seq_action) < 100);
        assert!(matches!(
            bundle.push_check(seq_action.clone()),
    ...
    fn flush_sanity_check() {
        ...
        let sized_seq_action = bundle.push_check(seq_action.clone()).unwrap();
        bundle.push(sized_seq_action);
    ...
    fn snapshot_bundle() -> SizedBundle {
        ...
        let sized_seq_action1 = bundle.push_check(seq_action1).unwrap();
        bundle.push(sized_seq_action1);
        let sized_seq_action1_2 = bundle.push_check(seq_action1_2).unwrap();
        bundle.push(sized_seq_action1_2);
        let sized_seq_action2 = bundle.push_check(seq_action2).unwrap();
        bundle.push(sized_seq_action2);
        bundle
    }
