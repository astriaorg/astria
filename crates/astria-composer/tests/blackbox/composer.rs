...
#[tokio::test]
async fn test_one_tx_integrity() {
    let test_composer = spawn_composer(&["test1"]).await;

    // blockhash, blocknumber and transactionindex are generated after execution - we can leave them out
    let txs = r#"[{
    ...
    let txs: Vec<Transaction> = serde_json::from_str(txs).unwrap();
    let mock_guard =
        mount_broadcast_tx_sync_mock(&test_composer.sequencer, "test1", 42, Some(txs.clone()))
            .await;
...
#[tokio::test]
async fn test_multiple_txs_integrity() {
    let test_composer = spawn_composer(&["test1"]).await;
...
async fn mount_broadcast_tx_sync_mock(
    ...
    expected_nonce: u32,
    expected_tx: Option<Vec<Transaction>>,
) -> MockGuard {
    use proto::{
        generated::sequencer::v1alpha1 as raw, native::sequencer::v1alpha1::SignedTransaction,
        Message as _,
    };
    let matcher = move |request: &Request| {
        ...
        if let Some(tx_data) = &expected_tx {
            let decoded_tx: Transaction = Rlp::new(&sequence_action.data).as_val().unwrap();
            println!("{}", json!(decoded_tx).to_string());
            assert_eq!(decoded_tx, tx_data[0])
        }
