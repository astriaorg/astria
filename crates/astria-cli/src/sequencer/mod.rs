...
impl Command {
    pub(super) async fn run(self) -> eyre::Result<()> {
        match self.command {
            ...
            SubCommand::ChangeTransferFee(change_transfer_fee) => change_transfer_fee.run().await,
        }
    }
}
...
enum SubCommand {
    ...
    /// Command for changing the transfer fee
    ChangeTransferFee(change_transfer_fee::Command),
}
