...
impl<C: ExecutionClient> Executor<C> {
    ...
    // TODO: display the sequencer block hash bytes nicely
    // #[instrument(ret, err, skip_all, fields(execution_block_hash =
    // hex::encode(&execution_block_hash), %sequencer_block_hash))]
    #[instrument(ret, err, skip_all, fields(execution_block_hash = hex::encode(&execution_block_hash)))]
    async fn finalize_block(
        &mut self,
        execution_block_hash: Vec<u8>,
        sequencer_block_hash: &Vec<u8>,
    ) -> Result<()> {
