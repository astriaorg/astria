...
/// A poor man's inclusive range to avoid converting heights to/from integers.
pub(crate) fn height_range_exclusive<T>(start: T, end: T) -> impl Iterator<Item = T>
where
    T: IncrementableHeight + PartialOrd + Copy,
{
    let start = (start < end).then_some(start);
...
mod test {
    ...
    #[test]
    fn exclusive_range_from_gives_1_to_3_gives_1_to_2() {
        let start: SequencerHeight = 1u32.into();
        let end = 3u32.into();
        let mut range = height_range_exclusive(start, end);
    ...
    #[test]
    fn exclusive_range_from_1_to_1_is_empty() {
        let start: SequencerHeight = 1u32.into();
        let mut range = height_range_exclusive(start, start);
        assert!(range.next().is_none());
    }

    #[test]
    fn exclusive_range_from_1_to_0_is_empty() {
        let start: SequencerHeight = 1u32.into();
