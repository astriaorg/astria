...
/// Tests if conductor restarts internal services if rollup response with `OUT_OF_RANGE`.
///
/// Astria Geth will return an `OutOfRange` status if `execute_block` is called for a block outside
/// of the bounds of the given session. If this happens, Conductor should restart and attempt to
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn restarts_on_out_of_range() {
    let test_conductor = spawn_conductor(CommitLevel::SoftAndFirm).await;
    ...
    // mount tonic `OutOfRange` error to cause the conductor to restart.
    // This mock can only be called up to 1 time, allowing a normal `execute_block` call after.
    let execute_block_tonic_code = mount_execute_block_tonic_code!(
        test_conductor,
        parent: "1",
        status_code: tonic::Code::OutOfRange,
    );
