...
impl serde::Serialize for PendingNonce {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("astria.sequencerblock.v1alpha1.PendingNonce", len)?;
        if self.inner != 0 {
...
impl<'de> serde::Deserialize<'de> for PendingNonce {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = PendingNonce;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct astria.sequencerblock.v1alpha1.PendingNonce")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<PendingNonce, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
        ...
        deserializer.deserialize_struct("astria.sequencerblock.v1alpha1.PendingNonce", FIELDS, GeneratedVisitor)
    }
}
