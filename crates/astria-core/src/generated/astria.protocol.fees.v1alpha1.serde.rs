...
impl serde::Serialize for ValidatorUpdateV2FeeComponents {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("astria.protocol.fees.v1alpha1.ValidatorUpdateV2FeeComponents", len)?;
        if let Some(v) = self.base.as_ref() {
            struct_ser.serialize_field("base", v)?;
        }
        if let Some(v) = self.multiplier.as_ref() {
...
impl<'de> serde::Deserialize<'de> for ValidatorUpdateV2FeeComponents {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = ValidatorUpdateV2FeeComponents;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct astria.protocol.fees.v1alpha1.ValidatorUpdateV2FeeComponents")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<ValidatorUpdateV2FeeComponents, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut base__ = None;
                ...
                Ok(ValidatorUpdateV2FeeComponents {
                    base: base__,
                    multiplier: multiplier__,
                })
            }
        }
        deserializer.deserialize_struct("astria.protocol.fees.v1alpha1.ValidatorUpdateV2FeeComponents", FIELDS, GeneratedVisitor)
    }
}
