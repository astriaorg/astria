...
/// Fees for all actions at genesis, which are all optional except for `fee_change`. If an action
/// has no associated fees, it will be assumed to be disabled until it is explicitly given fees
/// via the `FeeChange` action. Transactions with deactivated actions will fail upon execution.
#[derive(Clone, Debug)]
pub struct GenesisFees {
    pub sequence: Option<SequenceFeeComponents>,
...
impl Protobuf for GenesisFees {
    ...
    fn try_from_raw_ref(raw: &Self::Raw) -> Result<Self, Self::Error> {
        ...
        let sequence = sequence
            .clone()
            .map(SequenceFeeComponents::try_from_raw)
            .transpose()
    ...
    fn to_raw(&self) -> Self::Raw {
        ...
        Self::Raw {
            ...
            sequence: sequence.map(|act| SequenceFeeComponents::to_raw(&act)),
