...
pub(in crate::auctioneer) struct AddressBytes<'a>(Cow<'a, [u8; ADDRESS_LEN]>);
...
impl<'a> From<AddressBytes<'a>> for crate::storage::StoredValue<'a> {
    fn from(address: AddressBytes<'a>) -> Self {
        crate::storage::StoredValue::Auctioneer(Value(ValueImpl::AddressBytes(address)))
    }
}
...
impl<'a> TryFrom<crate::storage::StoredValue<'a>> for AddressBytes<'a> {
    ...
    fn try_from(value: crate::storage::StoredValue<'a>) -> Result<Self, Self::Error> {
        let crate::storage::StoredValue::Auctioneer(Value(ValueImpl::AddressBytes(address))) =
            value
        else {
