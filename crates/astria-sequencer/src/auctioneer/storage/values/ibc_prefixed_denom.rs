...
pub(in crate::auctioneer) struct IbcPrefixedDenom<'a>(Cow<'a, [u8; 32]>);
...
impl<'a> From<IbcPrefixedDenom<'a>> for crate::storage::StoredValue<'a> {
    fn from(denom: IbcPrefixedDenom<'a>) -> Self {
        crate::storage::StoredValue::Auctioneer(Value(ValueImpl::IbcPrefixedDenom(denom)))
    }
}
...
impl<'a> TryFrom<crate::storage::StoredValue<'a>> for IbcPrefixedDenom<'a> {
    ...
    fn try_from(value: crate::storage::StoredValue<'a>) -> Result<Self, Self::Error> {
        let crate::storage::StoredValue::Auctioneer(Value(ValueImpl::IbcPrefixedDenom(denom))) =
            value
        else {
            bail!(
