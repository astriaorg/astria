...
async fn ensure_correct_block_fees_transfer() {
    ...
    let transfer_base = 1;
    state
        .put_fees(TransferFeeComponents::new(transfer_base, 0))
        .unwrap();
...
async fn ensure_correct_block_fees_sequence() {
    ...
    let mut state = StateDelta::new(snapshot);
    state
        .put_fees(RollupDataSubmissionFeeComponents::new(1, 1))
        .unwrap();
...
async fn ensure_correct_block_fees_init_bridge_acct() {
    ...
    state
        .put_fees(InitBridgeAccountFeeComponents::new(
...
async fn ensure_correct_block_fees_bridge_lock() {
    ...
    state
        ...
        .unwrap();
    state
        .put_fees(BridgeLockFeeComponents::new(
            transfer_base,
...
async fn ensure_correct_block_fees_bridge_sudo_change() {
    ...
    state
        .put_fees(BridgeSudoChangeFeeComponents::new(sudo_change_base, 0))
        .unwrap();
    state
...
async fn bridge_lock_fee_calculation_works_as_expected() {
    ...
    state
        .put_fees(TransferFeeComponents::new(transfer_fee, 0))
        .unwrap();
    state
        .put_fees(BridgeLockFeeComponents::new(transfer_fee, 2))
        .unwrap();
