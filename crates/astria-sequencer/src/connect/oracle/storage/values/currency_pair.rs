...
pub(in crate::connect::oracle) struct CurrencyPair<'a> {
    base: Cow<'a, str>,
    quote: Cow<'a, str>,
}
...
impl<'a> From<CurrencyPair<'a>> for crate::storage::StoredValue<'a> {
    fn from(currency_pair: CurrencyPair<'a>) -> Self {
        crate::storage::StoredValue::ConnectOracle(Value(ValueImpl::CurrencyPair(currency_pair)))
    }
}
...
impl<'a> TryFrom<crate::storage::StoredValue<'a>> for CurrencyPair<'a> {
    ...
    fn try_from(value: crate::storage::StoredValue<'a>) -> Result<Self, Self::Error> {
        let crate::storage::StoredValue::ConnectOracle(Value(ValueImpl::CurrencyPair(
            currency_pair,
        ))) = value
        else {
            bail!(
                "connect oracle stored value type mismatch: expected currency pair, found \
                 {value:?}"
            );
        };
        Ok(currency_pair)
