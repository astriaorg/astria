...
impl<'a> From<MarketMap<'a>> for crate::storage::StoredValue<'a> {
    fn from(market_map: MarketMap<'a>) -> Self {
        crate::storage::StoredValue::ConnectMarketMap(Value(ValueImpl::MarketMap(market_map)))
    }
}
...
impl<'a> TryFrom<crate::storage::StoredValue<'a>> for MarketMap<'a> {
    ...
    fn try_from(value: crate::storage::StoredValue<'a>) -> Result<Self, Self::Error> {
        let crate::storage::StoredValue::ConnectMarketMap(Value(ValueImpl::MarketMap(market_map))) =
            value
        else {
            bail!(
                "connect market map stored value type mismatch: expected market map, found \
                 {value:?}"
            );
        };
        Ok(market_map)
