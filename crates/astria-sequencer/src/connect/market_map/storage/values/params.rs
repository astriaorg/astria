...
use astria_core::{
    connect::market_map::v2::Params as DomainParams,
    primitive::v1::{
        Address,
        ADDRESS_LEN,
    },
...
impl<'a> From<Params<'a>> for crate::storage::StoredValue<'a> {
    fn from(params: Params<'a>) -> Self {
        crate::storage::StoredValue::ConnectMarketMap(Value(ValueImpl::Params(params)))
    }
}
...
impl<'a> TryFrom<crate::storage::StoredValue<'a>> for Params<'a> {
    ...
    fn try_from(value: crate::storage::StoredValue<'a>) -> Result<Self, Self::Error> {
        let crate::storage::StoredValue::ConnectMarketMap(Value(ValueImpl::Params(params))) = value
        else {
            bail!(
                "connect market map stored value type mismatch: expected params, found {value:?}"
            );
        };
        Ok(params)
    }
