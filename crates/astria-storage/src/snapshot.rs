...
impl StateRead for Snapshot {
    ...
    fn prefix_raw(&self, prefix: &str) -> Self::PrefixRawStream {
        ...
        // We need to first prefix range the key preimages column family, then use the hashed
        // matches to fetch the values from the JMT column family.
        tokio::task::Builder::new()
            ...
            .spawn_blocking(move || {
                span.in_scope(|| {
                    ...
                    for tuple in jmt_keys_iterator {
                        // For each key that matches the prefix, fetch the value from the JMT column
                        // family.
            ...
            .spawn_blocking(move || {
                span.in_scope(|| {
                    ...
                    for i in iter {
                        ...
                        // This is a bit of a hack, but RocksDB doesn't let us express the "prefixed
                        // range-queries", that we want to support. In
                        // particular, we want to be able to do a prefix query that starts
                        // at a particular key, and does not have an upper bound. Since we can't
                        // create an iterator that cover this range, we have
                        // to filter out the keys that don't match the prefix.
    ...
    fn object_get<T: Any + Send + Sync + Clone>(&self, _key: &str) -> Option<T> {
        // No-op -- this will never be called internally, and `Snapshot` is not exposed in public
        // API
        None
    }
