...
impl<TFormat, TBytes, TPrefix> Builder<TFormat, TBytes, TPrefix> {
    ...
    pub fn with_iter<T: IntoIterator<Item = u8>>(
        self,
        iter: T,
    ) -> Builder<TFormat, WithBytes<'static, T>, TPrefix> {
        Builder {
    ...
    pub fn try_build(self) -> Result<Address<TFormat>, Error> {
        ...
        let bytes = match bytes {
            ...
            WithBytesInner::Slice(bytes) => <[u8; ADDRESS_LENGTH]>::try_from(bytes.as_ref())
                .map_err(|_| Error::incorrect_address_length(bytes.len()))?,
        };
...
fn try_collect_to_array<I: IntoIterator<Item = u8>>(
    iter: I,
) -> Result<[u8; ADDRESS_LENGTH], Error> {
    let mut arr = [0; ADDRESS_LENGTH];
    let mut iter = iter.into_iter();

    let mut bytes_set = 0;
    for (i, byte) in iter.by_ref().take(ADDRESS_LENGTH).enumerate() {
        arr[i] = byte;
        bytes_set += 1;
    }
    if bytes_set < ADDRESS_LENGTH {
        return Err(Error::incorrect_address_length(bytes_set));
    } else if iter.next().is_some() {
        return Err(Error::too_many_bytes());
    }
    Ok(arr)
}
...
impl Error {
    ...
    fn incorrect_address_length(received: usize) -> Self {
        Self(ErrorKind::IncorrectLength {
            received: BytesProvided::Exact(received),
        })
    }

    fn too_many_bytes() -> Self {
        Self(ErrorKind::IncorrectLength {
            received: BytesProvided::MoreThan(ADDRESS_LENGTH),
        })
    }
}
...
enum ErrorKind {
    ...
    IncorrectLength { received: BytesProvided },
...
impl Display for BytesProvided {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BytesProvided::Exact(bytes) => write!(f, "{bytes}"),
            BytesProvided::MoreThan(bytes) => write!(f, "more than {bytes}"),
        }
    }
}
...
mod tests {
    ...
    fn assert_wrong_address_bytes(bad_account: &[u8]) {
        ...
        assert_eq!(BytesProvided::Exact(bad_account.len()), received);
    }
