...
without the EVM Bridge Withdrawer Enabled.
...
from helpers.astria_cli import Cli, SEQUENCER_BRIDGE_SIGNING_KEY
...
from helpers.docker_volume_controller import DockerVolumeController
...
parser = argparse.ArgumentParser(prog="smoke_cli", description="Runs the smoke CLI test.")
...
print(colored("Running Astria Stack smoke CLI test", "light_blue"))
...
deploy_evm_fn = lambda evm_node: evm_node.deploy_rollup(image_controller, bridge_withdrawer_enabled=False)
...
evm.wait_until_balance(EVM_DESTINATION_ADDRESS, expected_balance, 30)
...
# Get the current block number for the collect-withdrawals command
print(colored("Getting current block number from EVM rollup...", "blue"))
current_block = evm.get_current_block_number()
print(colored(f"Current block number: {current_block}", "green"))

# Using DockerVolumeController to handle key and output files
print(colored("Setting up Docker volume for withdrawal processing...", "blue"))
volume = DockerVolumeController("cli-test-withdrawals")

# Write the signing key to the volume
print(colored("Writing signing key to Docker volume...", "blue"))
volume.map_and_execute_data(["alpine", "sh", "-c", f"echo '{SEQUENCER_BRIDGE_SIGNING_KEY}' > /data/key"])

# Collect withdrawals
print(colored("Collecting withdrawals from rollup...", "blue"))
volume.map_and_execute_astria([
    "--network", "host",
    f"ghcr.io/astriaorg/astria-cli:{cli_image}",
    "bridge", "collect-withdrawals",
    f"--rollup-endpoint", ETH_WS_URL,
    f"--contract-address", EVM_CONTRACT_ADDRESS,
    "--from-rollup-height", "1",
    f"--to-rollup-height", str(current_block),
...
# Submit withdrawals
print(colored("Submitting withdrawals to sequencer...", "blue"))
volume.map_and_execute_astria([
    "--network", "host",
    f"ghcr.io/astriaorg/astria-cli:{cli_image}",
    "bridge", "submit-withdrawals",
    "--signing-key", "/astria/key",
    f"--sequencer-chain-id", SEQUENCER_CHAIN_ID,
    f"--sequencer-url", SEQUENCER_RPC_URL,
    "--input", "/astria/tempfile"
])

# Check withdrawal balance on Sequencer
...
print(colored("Smoke CLI test successful", "green"))
